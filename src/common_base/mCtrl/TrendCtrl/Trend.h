    ...
    void CTrend<Tx, Ty, WAVE_NUM, POINT_NUM>::Draw(QPainter* painter, const QRect& rect)
    {
        ...
        for(int num = 0; num < WAVE_NUM; num++) {
            for(int i = 0; i < len; i++) {
                ...
                if (m_pPointBuf[i].x() > rect.right()) {
                    m_pPointBuf[i].setX(rect.right());
                }
                else if (m_pPointBuf[i].x() < rect.left()) {
                    m_pPointBuf[i].setX(rect.left());
                }
                if (m_pPointBuf[i].y() > rect.bottom()) {
                    m_pPointBuf[i].setY(rect.bottom());
                }
                else if (m_pPointBuf[i].y() < rect.top()) {
                    m_pPointBuf[i].setY(rect.top());
                }
            }
